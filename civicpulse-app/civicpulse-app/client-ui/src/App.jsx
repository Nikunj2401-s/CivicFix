...
import {
  backIssue, checkLand, createIssue, verifyIssue, signInWithGoogle, getPolicy, assetUrl, findNearbyIssues, getCurrentUser, getIssues, getMyReports,
    ...
    <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <section className={`modal standing-${standing.key}`} role="dialog" aria-modal="true" aria-labelledby="issue-detail-title" data-testid="modal-issue-detail">
        ...
        <div className="modal-body">
          ...
          <div className="issue-photo" data-testid={`photo-issue-${issue.id}`}>{issue.photo_url ? <img src={assetUrl(issue.photo_url)} alt={`Street evidence for ${issue.id}`} /> : <div><CloudUpload size={20} /><strong>No photo attached</strong><span>Residents can add a street-level photo when filing.</span></div>}</div>
